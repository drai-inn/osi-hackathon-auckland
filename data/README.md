...
| `manifest/benchmark_v1.csv` | The real manifest, produced by WP-A | **does not exist yet** — [S0](../docs/02-pipeline/stages/S0-benchmark.md) |
...
- **activity reported under different assay types across targets** — a selectivity ratio built
  from mixed assays is an artefact, not a measurement ([C1](../docs/01-scope/open-questions.md))
...
- no `non_selective` compounds — without discriminating negatives, enrichment may be achievable on
  molecular weight alone ([R6](../docs/05-feasibility/risks.md))
...
(gitignored) with the layout in [interfaces.md](../docs/02-pipeline/interfaces.md#layout). Storage
plan is WP-F's; budget ~1 TB for the hackathon minimum `[estimate]`.

## Licensing

If we intend to release the benchmark ([E2](../docs/01-scope/open-questions.md)), every source must
permit it. ChEMBL is CC BY-SA 3.0, which is share-alike — check the implications before mixing
sources. See [data-sources.md](../docs/05-feasibility/data-sources.md).
