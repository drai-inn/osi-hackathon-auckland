...
| Token | Value | Use |
| --- | --- | --- |
| `--south` | `#00D3F6` | The Auckland hub accent — **the UoA cyan**. Southern Cross, the "two days ahead" badge. Little else |
...
Six sheared bars in two opposed rows. It reads as a woven pattern, and it is small by design: **a
lead-in before a heading, or a separator between sections.** Not a background, not a texture, not
scaled up to hero size.

```html
<img src="brand/uoa-motif.svg" alt="" width="36" height="18">   <!-- lead-in -->
```

**Do not restyle it.** Not recoloured, not rearranged, not reproportioned, not animated. It is an
institutional mark with design heritage that is not ours to reinterpret, and the correct way to use
it is exactly as supplied.
...
6. **The UoA motif is a lead-in, not decoration.** One per section at most, at its supplied size.
